...
const getTeams = async(): Promise<Team[]> => {  
  const response =await api.get<Team[]>('api/Team/Get');
  return response.data;
};

const getTeamById = async(id: number): Promise<Team> => {
  const response = await api.get<Team>(`api/Team/GetById/${id}`);
  return response.data;
};
...
const createTeam = async(teamData: Omit<Team,'teamId'>): Promise<Team> => {
  const response = await api.post<Team>('api/Team/Create', teamData);
  return response.data;
};

const updateTeam = async(userData: Team) : Promise<Team> => {
  const response = await api.put<Team>(`api/Team/Update`, userData);
  return response.data;
};
