...
const getEmployeeProject = async(): Promise<EmployeeProject[]> => {
    const response = await api.get<EmployeeProject[]>('api/EmployeeProject/Get');
    return response.data;
};

const getEmployeeProjectByEmployeeId = async(id: number): Promise<EmployeeProject> => {
    const response = await api.get<EmployeeProject>(`api/EmployeeProject/GetByEmpoyeeId/${id}`);
    return response.data;
}

const getEmployeeProjectByProjectId = async(id : number): Promise<EmployeeProject> => {
    const response = await api.get<EmployeeProject>(`api/EmployeeProject/GetByProjectId/${id}`);
    return response.data;
};

const createEmployeeProject = async(employeeProjectData: Omit<EmployeeProject,'employeeProjectId'>): Promise<EmployeeProject> => {
    const response = await api.post<EmployeeProject>('api/EmployeeProject/Create', employeeProjectData);
    return response.data;
}

const updateEmployeeProject = async(employeeProjectData : EmployeeProject): Promise<EmployeeProject> => {
    const response = await api.put<EmployeeProject>(`api/EmployeeProject/Update`, employeeProjectData);
    return response.data;
}
